...
if (isset($_GET['advert_id'])) {
    ...
    echo json_encode(SQLManager::findAll('*','adverts WHERE end_date >= NOW() ORDER BY id DESC LIMIT 10'));
} else if (intval($_GET['page'])) {
    // Récupère les 10 x $_GET['page'] sans prendre ceux des pages précédentes
    $page_number = ($_GET['page'] - 1) * 10;
    echo json_encode(SQLManager::findAll('*','adverts WHERE end_date >= NOW() ORDER BY id DESC LIMIT '.$page_number.', 10'));
} else {
